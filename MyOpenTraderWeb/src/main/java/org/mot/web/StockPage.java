...
import org.mot.web.chart.ScatterTickChart;
...
public class StockPage extends WebPage {
	...
	public StockPage(final PageParameters parameters) {
		...
		try {
			...
			//add(new ScatterTickChart("scatterTicks", symbol, config.getInt("chart.todays.refreshMsec")));
			add(new TickRangeChart("scatterTicks", symbol, config.getInt("chart.todays.refreshMsec")));
			add(new RealTimeTickChart("realTimeChart", symbol, config.getInt("page.mvaStockPage.showLastTicks"), config.getInt("chart.realtime.refreshMsec"))); 

...
public class TodaysTickChart extends Chart {
	...
	public TodaysTickChart(String name, final String symbol, int refresh) {
		...
		Order[] BUYorders = new OrderDAO().getAllOrdersBySymbol(this.symbol,
				"BUY");
		...
		if (BUYorders.length > 0) {
			...
			for (int i = 0; i < BUYorders.length; i++) {
				buyData.add(new Coordinate<Long, Double>(db
						.convertTimestampToLong(BUYorders[i].getTimestamp()),
		...
		// Add the sell orders
		//
		Order[] SELLorders = new OrderDAO().getAllOrdersBySymbol(this.symbol,
				"SELL");
		...
		if (SELLorders.length > 0) {
			...
			for (int i = 0; i < SELLorders.length; i++) {
				sellData.add(new Coordinate<Long, Double>(db
						.convertTimestampToLong(SELLorders[i].getTimestamp()),

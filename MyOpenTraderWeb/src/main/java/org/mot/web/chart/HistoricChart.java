...
public class HistoricChart extends Chart {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(HistoricChart.class);
	...
	public HistoricChart(String name, String stock, int maxCount, int RefreshSeconds) {
	...
	// Historic Chart for EUR & USD
			...
			// Disable exporting
			hcOptions.setExporting(new ExportingOptions()
			...
			TickHistory[] historyPrices = sdw.getHistoricPriceTableForStock(stock, "1 day", maxCount);
			...
			//(String stock, int quantity, int mvgAvg1, int mvgAvg2, Double[] prices)
			int ma1 = 3;
			int ma2 = 6;
			String time = " day ";
			...
			for (int i = 0; i < historyPrices.length; i++ ){
					...
					seriesData.add(new Coordinate<Long, Double>(historyPrices[i].getDateAsLong(), historyPrices[i].getClose()));
					
			}
			
			hcOptions.setyAxis(new Axis()
			.setTitle(new Title("Price"))
			.setMin(0)
			);
		
		// Finally managed to get the x axis labelled correctly! Woot woot woot!
		hcOptions.setxAxis(new Axis()
			...
			/*
			CustomCoordinatesSeries<Long,Double> series2 = new CustomCoordinatesSeries<Long,Double>();
			series2.setType(SeriesType.LINE);
			series2.setName(ma1 + time + " Moving Average");
			series2.setPointInterval(50);
			series2.setPointStart(0);
			
			List<Coordinate<Long, Double>> seriesData2 = new ArrayList<Coordinate<Long, Double>>();
			
			for (int i = 0; i < historyPrices.length; i++ ){
				
					seriesData2.add(new Coordinate<Long, Double>(historyPrices[i].getDateAsLong(), mv1[i]));
			...
			CustomCoordinatesSeries<Long,Double> series3 = new CustomCoordinatesSeries<Long,Double>();
			series3.setType(SeriesType.LINE);
			series3.setName(ma2 + time +  " Moving Average");
			series3.setPointInterval(50);
			series3.setPointStart(0);
			
			List<Coordinate<Long, Double>> seriesData3 = new ArrayList<Coordinate<Long, Double>>();
			
			for (int i = 0; i < historyPrices.length; i++ ){
				
					seriesData3.add(new Coordinate<Long, Double>(historyPrices[i].getDateAsLong(), mv2[i]));
			...
			*/
			...
			//super.add(new AjaxSelfUpdatingTimerBehavior(Duration.seconds(RefreshSeconds)));

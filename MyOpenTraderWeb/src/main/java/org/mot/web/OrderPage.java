	...
	public OrderPage(final PageParameters parameters) {
		...
		try {
			...
			if (strategy == null) {
				...
				// If searching for a particular strategy, only return those
				// orders
			...
			columns2.add((IColumn<?, ?>) new PropertyColumn(new Model<String>(
					"Symbol"), "symbol"));
			columns2.add((IColumn<?, ?>) new PropertyColumn(new Model<String>(
					"Quantity"), "quantity"));
			columns2.add((IColumn<?, ?>) new PropertyColumn(new Model<String>(
			...
			DefaultDataTable table = new DefaultDataTable("closedOrderTable",
					columns2, new SortableClosedOrderDataProvider(), 20);
